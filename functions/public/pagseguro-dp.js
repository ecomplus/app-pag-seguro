...
(function () {
  ...
  window.pagseguroSenderHash = function () {
    return new Promise(function (resolve, reject) {
      PagSeguroDirectPayment.onSenderHashReady(function (response) {
        if (checkResponse(response)) {
          ...
          reject(new Error('Error with sender hash'))
        }
      });
    })
  }

  window._senderHash = new Promise(function (resolve, reject) {
    window.pagseguroSenderHash().then(function (hash) {
        return resolve({ intermediator_buyer_id: hash })
      }).catch(reject)
  })
